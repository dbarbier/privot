...
{
public:
  explicit TestObject() : Beta(2., 5., -1., 2.) {}
  explicit TestObject(const String & name) : Beta(2., 5., -1., 2.)
  {
    setName(name);
  }
  virtual ~TestObject() {}
  ...
  try
    {
      ...
      Beta::NumericalPointWithDescriptionCollection parameters = distribution.getParametersCollection();
      ...
      Beta newDistribution(mu, sigma, distribution.getA(), distribution.getB(), Beta::MUSIGMA);
      fullprint << "r from (mu, sigma)=" << newDistribution.getR() << std::endl;
      fullprint << "t from (mu, sigma)=" << newDistribution.getT() << std::endl;
    }
  catch (TestFailed & ex)
    {
