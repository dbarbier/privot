...
{
public:
  explicit TestObject() : Gamma(1.5, 2.5, -0.5) {}
  explicit TestObject(const String & name) : Gamma(1.5, 2.5, -0.5)
  {
    setName(name);
  }
  virtual ~TestObject() {}
  ...
  try
    {
      ...
      Gamma distribution(1.5, 2.5, -0.5);
      ...
      Gamma::NumericalPointWithDescriptionCollection parameters = distribution.getParametersCollection();
      ...
      Gamma newDistribution(mu, sigma, distribution.getGamma(), Gamma::MUSIGMA);
      fullprint << "k from (mu, sigma)=" << newDistribution.getK() << std::endl;
      fullprint << "lambda from (mu, sigma)=" << newDistribution.getLambda() << std::endl;
    }
  catch (TestFailed & ex)
    {
