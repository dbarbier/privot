//                                               -*- C++ -*-
/**
 *  @file  SimulationSensitivityAnalysis.cxx
    ...
    threshold_()
{
  ...
  if (!model.isHistoryEnabled()) throw InvalidArgumentException(HERE) << "Error: cannot perform analysis if the history of the underlying model is disable.";
  inputSample_  = model.getInputHistory().getSample();
  // Check if the samples are not empty
  ...
  outputSample_ = model.getOutputHistory().getSample();
...
{
  NumericalPointWithDescription result(transformation_(computeMeanPointInEventDomain(threshold)).normalize2());
...
Graph SimulationSensitivityAnalysis::drawImportanceFactorsRange(const Bool probabilityScale) const
{
  ...
  // Now, we can go through the data and accumulate the importance factors. If we just call the computeImportanceFactors() method directly, the cost is O(size^2), which is too expansive for typical situations.
      ...
      if (mustDraw)
        {
          ...
          try
            {
              importanceFactors = transformation_(accumulator / accumulated).normalize2();
  ...
  const Description colors(Curve::GetValidColors());
  const UnsignedLong maxColors(colors.getSize());
  for (UnsignedLong j = 0; j < inputDimension; ++j)
    {
      Curve curve(dataCollection[j]);
      curve.setColor(colors[j % maxColors]);
      curve.setLegendName(inputSample_.getDescription()[j]);
      graph.add(curve);
    }
  // Highlight the default threshold importance factors if stable enough
  if ((good >= sampleMargin) && (good < size - sampleMargin))
    {
      ...
      curve.setColor("red");
      curve.setLegendName("current thres.");
      graph.add(curve);
    }
  return graph;
}
