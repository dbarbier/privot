//                                               -*- C++ -*-
/**
 *  @file  ARMALikelihoodFactory.cxx
 ...
 *  Id:      $Id: ARMALikelihoodFactory.cxx 2392 2012-02-17 18:35:43Z schueller $
 */
...
NumericalScalar ARMALikelihoodFactory::computeLogLikelihood(const NumericalPoint & beta) const
{
  ...
  NumericalScalar norm2(vector_h.norm2());

  NumericalScalar f1(norm1 - norm2);
  ...
  const NumericalScalar n(static_cast<double>(w_.getSize()));
  const NumericalScalar fullSize(static_cast<double>(dimension_ * n));
  const NumericalScalar lg2pi(log(2.0 * M_PI));
  const NumericalScalar detQ(covarianceMatrix_.computeDeterminant());
  ...
  if (p > 0)
    {
      ...
      const Collection<NumericalComplex> eigenValues(matrix.computeEigenValues(false));
...
ARMA * ARMALikelihoodFactory::build(const TimeSeries & timeSeries) const
{
  ...
  return ARMA(phi, theta, whiteNoise).clone();
}
...
void ARMALikelihoodFactory::defaultInitialize() const
{
  ...
  const NumericalSample values(w_.getNumericalSample());
  ...
  for (UnsignedLong d = 0; d < dimension_; ++d)
    {
      ...
      const NumericalScalar sigma(whiteNoise.getDistribution().getStandardDeviation()[0]);
...
SquareMatrix ARMALikelihoodFactory::computeV1_Omega_V1T_Cholesky() const
{
  ...
  NumericalScalar smallestValue(1.0);
