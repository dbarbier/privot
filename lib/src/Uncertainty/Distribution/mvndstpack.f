...
*
************************************
* COMENTED FOR OPENTURNS INCLUSION *
************************************
*      PROGRAM TSTNRM
*
...
*      DO K = 1, 3
*         PRINT '(/'' I     Limits'')'
*         PRINT '(4X,''Lower  Upper  Lower Left of
*                 Correlation Matrix'')'
*         IJ = 0
*         DO I = 1, N
...
*            should be stored in CORREL( J + ((I-2)*(I-1))/2 ),
*            for J < I.
...
*
     ...
     &                 ERROR, VALUE, E, D, MVNDNT, MVNDFN, RC
      COMMON /DKBLCK/IVLS
      IF ( N .GT. 500 .OR. N .LT. 1 ) THEN
         ...
         ERROR = 1
      ELSE
         RC = MVNDNT(N, CORREL, LOWER, UPPER, INFIN, INFIS, D, E)
...
************************************************************************
 ...
 10   VK(1) = ONE/P(NP)
      ...
      DO I = 2, NDIM
         IF ( I .LE. KLIM ) THEN
            K = INT(MOD( C(NP, MIN(NDIM-1,KLIM-1))*DBLE(K),
     &              DBLE(P(NP)) ))
            VK(I) = K*VK(1)
         ELSE
