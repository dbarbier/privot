...
String WrapperFrameworkData::__str__(const String & offset) const
{
  OSS oss;
  oss << offset << "studyid       : " << studyid_ << "\n"
      << offset << "studycase     : " << studycase_ << "\n"
      << offset << "componentname : " << componentname_;
  return oss;
}
...
{
  struct FrameworkData * framework = newC<FrameworkData>();

  framework->studyid_       = p_framework_->studyid_;
  framework->studycase_     = strdup( p_framework_->studycase_.c_str() );
  framework->componentname_ = strdup( p_framework_->componentname_.c_str() );
...
{
  free( framework->studycase_ );
  free( framework->componentname_ );
  deleteC( framework );
}
...
struct WrapperExchangedData * WrapperData::getNewWrapperExchangedDataForCInterface() const
{
  ...
  p_exchangedData->framework_    = getNewFrameworkForCInterface();

  p_exchangedData->platform_->generalTemporaryDirectory_ = strdup( Path::GetTemporaryDirectory().c_str() );;
  p_exchangedData->platform_->realRegexpShortcut_        = ResourceMap::GetAsNewCharArray("real-regexp-shortcut");
...
void WrapperData::FreeWrapperExchangedDataForCInterface(const struct WrapperExchangedData * exchangedData)
{
  ...
  // ... framework
  WrapperData::FreeFrameworkForCInterface( exchangedData->framework_ );
