  ...
  : std::vector<String>(WrapperObject::FunctionPrefixSize)
{
  ...
  ref_me[WrapperObject::EXECUTION_TIMESERIES] = "exec_timeseries_";
...
struct WrapperSymbols : public Object
{
  ...
  ExecutionTimeSeriesFunctionPointer execTimeSeriesSymbol_;
    ...
    if (ResourceMap::Get("computation-progression-visible") == "true")
      {
        ...
        double ratio = 100. * args->count / args->size;
        int width = static_cast<int>( ceil( log10( args->size ) ) );
        LOGINFO( OSS() << "Progression : "
                 << std::setw(7) << ratio << "% ( "
  ...
  enum WrapperErrorCode defaultWrapperExecTimeSeriesFunction(void * p_state,
                                                             const struct timeseries * inTimeSeries,
                                                             struct timeseries * outTimeSeries,
                                                             const struct WrapperExchangedData * p_exchangedData,
                                                             void * p_error) const
  {
    // #ifdef HAVE_TBB
    ...
    wrapperInfo_(0)
{
  ...
  wrapperSymbols_->execTimeSeriesSymbol_ = REINTERPRET_CAST(ExecutionTimeSeriesFunctionPointer,
                                                            getSymbol(getFunctionName(EXECUTION_TIMESERIES), optional, 0) );
...
{
  return OSS(false) << offset << getName() << " -> " << handle_.__str__();
}
...
/* Method execute actually realizes the desired treatment */
/* TODO: MAKE THIS ROUTINE THREAD-SAFE */
TimeSeries WrapperObject::execute(void * p_state,
                                  const TimeSeries & inTS) const
{
  const UnsignedLong inDimension = getInNumericalPointDimension(p_state);
  // Check the inTS argument
  if (inTS.getDimension() != inDimension)
    throw InvalidArgumentException(HERE)
      << "Argument 'inTS' has incorrect dimension (" << inTS.getDimension()
      << "). Expected = " << inDimension;

  const UnsignedLong size = inTS.getSize();
  const TimeSeriesImplementation & inTSi = * inTS.getImplementation();

  // We create a point structure to embed the in NumericalPoint passed as argument
  struct timeseries inTimeSeries;
  inTimeSeries.size_      = size;
  inTimeSeries.dimension_ = inDimension;
  inTimeSeries.data_      = const_cast<NumericalScalar *>( &inTSi[0][0] );

  // We create the output sample
  const UnsignedLong outDimension = getOutNumericalPointDimension(p_state);
  TimeSeries outTS(size, outDimension);
  TimeSeriesImplementation & outTSi = * outTS.getImplementation();

  // We create a point structure to embed the returned outS NumericalSample
  struct timeseries outTimeSeries;
  outTimeSeries.size_      = size;
  outTimeSeries.dimension_ = outDimension;
  outTimeSeries.data_      = &outTSi[0][0];
  ...
  try
    {
      returnCode = (wrapperSymbols_->execTimeSeriesSymbol_ != 0) ?
        (*(wrapperSymbols_->execTimeSeriesSymbol_))( p_state, & inTimeSeries, & outTimeSeries, p_exchangedData_, error_.get() ) :
        wrapperSymbols_->defaultWrapperExecTimeSeriesFunction( p_state, & inTimeSeries, & outTimeSeries, p_exchangedData_, error_.get() ) ;
      if (returnCode != WRAPPER_OK)
        throw WrapperInternalException(HERE) << "Wrapper function '"
                                             << getFunctionName(EXECUTION_TIMESERIES)
                                             << "' returned error Message: "
                                             << wrapper_getErrorAsString(returnCode)
                                             << ". Reason: " << getWrapperError( error_.get() );
    }
    ...
    {
      LOGINFO(OSS() << "Time series evaluation function returned an error. Reason: " << ex ) ;
      throw ex;
    }
