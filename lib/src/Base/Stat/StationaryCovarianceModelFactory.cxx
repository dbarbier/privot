//                                               -*- C++ -*-
/**
 *  @file  StationaryCovarianceModelFactory.cxx
...
UserDefinedStationaryCovarianceModel StationaryCovarianceModelFactory::build(const SpectralModel & mySpectralModel) const
{
  ...
  for (UnsignedLong k = 0; k < size; ++k)
    {
      ...
      const HermitianMatrix spectralDensity(mySpectralModel.computeSpectralDensity(currentFrequency));
...
UserDefinedStationaryCovarianceModel * StationaryCovarianceModelFactory::build(const ProcessSample & sample) const
{
  // First build the spectral model
  SpectralModel mySpectralModel(spectralFactory_.build(sample));
  // Then inverse the formula in order to get the covariance model
  return build(mySpectralModel).clone();
}

UserDefinedStationaryCovarianceModel * StationaryCovarianceModelFactory::build(const TimeSeries & timeSeries) const
{
  // First build the spectral model
  SpectralModel mySpectralModel(spectralFactory_.build(timeSeries));
  // Then inverse the formula in order to get the covariance model
  return build(mySpectralModel).clone();
}
