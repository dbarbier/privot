...
{
  for (UnsignedLong i = 0; i < getDimension(); ++i) if (lowerBound_[i] > upperBound_[i]) return true;
  return false;
}
...
NumericalScalar Interval::getNumericalVolume() const
{
  if (isNumericallyEmpty()) return 0.0;
  NumericalScalar volume(1.0);
  for (UnsignedLong i = 0; i < getDimension(); ++i)
