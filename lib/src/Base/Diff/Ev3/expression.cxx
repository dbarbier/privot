...
#define ISINTTOLERANCE 1e-8
  ...
  Expression operator * (Expression a,
                         Expression t)
  {
    // std:cerr << "in operator *, a=" << a->ToString() << ", t=" << t->ToString() << std::endl;
    Expression ret;
    // make a preliminary check
        ...
        else
          {
            ...
            switch(op)
              ...
              case ATAN:
                ...
                c->One();
                ret = ret + c;
                ret = Cosh(Diff(a->GetNode(0), vi)) / ret; // f' / (f^2 + 1)
                // std:cerr << "\nout case ATAN\n" << std::endl;
                break;
              case SINH:
                // std:cerr << "\nin case SINH\n" << std::endl;
              ...
              case ERF:
                ...
                ret = M_2_SQRTPI * Diff(a->GetNode(0), vi) * Exp(-(a->GetCopyOfNode(0) ^ two)); // (2/sqrt(pi)) f'e^(-f^2)
                // std:cerr << "\nout case ERF\n" << std::endl;
                break;
              case ERFC:
                // std:cerr << "\nin case ERFC\n" << std::endl;
              ...
              case ABS:
                ...
                ret = Sign(a->GetCopyOfNode(0)); // sign(f)
                // std:cerr << "\nout case ABS\n" << std::endl;
                break;
              default:
                // std:cerr << "\nin case default\n" << std::endl;
