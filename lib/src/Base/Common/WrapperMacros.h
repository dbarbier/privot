 ...
 *
 * @author dutka
 * @date   2007-11-06 19:01:18 +0100 (mar, 06 nov 2007)
 */
...
#define INTIMESERIES_ARRAY  (inTimeSeries->data_)
#define OUTTIMESERIES_ARRAY (outTimeSeries->data_)

#define INTIMESERIES_TIME( i )  (inTimeSeries->data_[  (i) * inTimeSeries->size_  ])
#define OUTTIMESERIES_TIME( i ) (outTimeSeries->data_[ (i) * outTimeSeries->size_ ])

#define INTIMESERIES_VALUES( i )  (&(inTimeSeries->data_[  (i) * inTimeSeries->size_  + 1 ]))
#define OUTTIMESERIES_VALUES( i ) (&(outTimeSeries->data_[ (i) * outTimeSeries->size_ + 1 ]))

#define INTIMESERIES_SIZE  (inTimeSeries->size_)
#define OUTTIMESERIES_SIZE (outTimeSeries->size_)

#define INTIMESERIES_DIMENSION  (inTimeSeries->dimension_)
#define OUTTIMESERIES_DIMENSION (outTimeSeries->dimension_)
...
#define FUNC_EXEC_TIMESERIES( name , code )     FUNC_EXEC_TIMESERIES_DEF( name, code )
#define FUNC_EXEC_TIMESERIES_DEF( name , code )                         \
  C_DECL                                                                \
  RETURNCODE func_exec_timeseries_ ## name (STATE p_state,              \
                                            INTIMESERIES inTimeSeries,  \
                                            OUTTIMESERIES outTimeSeries, \
                                            EXCHANGEDDATA p_exchangedData, \
                                            ERROR p_error)              \
  {                                                                     \
    const char FUNCTIONNAME[] UNUSED = "func_exec_timeseries_" #name ;  \
    dbg_printEntrance(FUNCTIONNAME);                                    \
    dbg_printState(FUNCTIONNAME, p_state);                              \
    dbg_printTimeseries(FUNCTIONNAME, inTimeSeries);                    \
                                                                        \
    code ;                                                              \
                                                                        \
    dbg_printTimeseries(FUNCTIONNAME, outTimeSeries);                   \
    dbg_printExit(FUNCTIONNAME);                                        \
                                                                        \
    return WRAPPER_OK;                                                  \
  }
