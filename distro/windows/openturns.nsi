...
OutFile "openturns-${PRODUCT_VERSION}-py27.exe"
...
Function .onInit
  ...
  ${If} $Python_INSTALL_PATH == ""
    MessageBox MB_OK|MB_ICONEXCLAMATION "Python 2.7 installation directory not found!$\rEnter manually the Python installation directory." /SD IDOK
  ...
  ${Else} 
    ...
    ; !insertmacro PRINT "Python 2.7 found in directory $Python_INSTALL_PATH."
  ${EndIf}
...
!ifndef DEBUG
  ...
  File /r "${OPENTURNS_PREFIX}\lib\python27\site-packages\openturns\*.*"

  ${ConfigWrite} "$OT_INSTALL_PATH\__init__.py"   "    OT_PYTHON_PATH = "   "$\"$OT_INSTALL_PATH$\""   $R0
  ...
  SetOutPath "$OT_INSTALL_PATH"
  ;  <R-executable-command      value="/home/souchaud/.wine/drive_c/R/R-2.12.0/bin/R.exe"         />
  ${ConfigWrite} "openturns.conf"   "  <R-executable-command      value="   "$\"$OT_INSTALL_PATH\R-2.12.0\bin\R.exe$\" />"   $R0
