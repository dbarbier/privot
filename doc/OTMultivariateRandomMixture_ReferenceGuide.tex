...
with~:
\begin{align}
  E_{m_1,\hdots,m_d}(k_1,\hdots,k_d)=e^{-i\sum_{j=1}^d k_jh_j\left(\mu_j+a\left(\frac{2m_j+1}{M}-1\right)\sigma_j\right)}
\end{align}
...
The evaluation of the $\Sigma^{-}$ part could be done using a FFT algorithm:
\begin{enumerate}
\item Pour $k\in\{0,\hdots,N-1\}$, we set:
\begin{align}
