...
/* The C function we want to be linked with is declared in this file */
...
*/
  ...
  FUNC_EXEC( WRAPPERNAME, {
      ...
      int rc = myCFunction( INPOINT_ARRAY, INPOINT_SIZE, OUTPOINT_ARRAY, OUTPOINT_SIZE );
      if (rc) {
	SETERROR( "Error in myCFunction. Function has returned code %d.", rc );
