...
try :
   ...
   # Test for sampling
    ...
    print "Point = ", repr(point), " pdf (ref)=%.6f" % pointPDFRef, " cdf (ref)=%.6f" % pointCDFRef
    ...
    for i in range(dim) :
        ...
        print "margin PDF (ref)=%.6f" % marginRef.computePDF( NumericalPoint(1,0.25))
        print "margin CDF      =%.6f" % margin.computeCDF(NumericalPoint(1,0.25))
        print "margin CDF (ref)=%.6f" % marginRef.computeCDF(NumericalPoint(1,0.25))
    ...
    print "margin PDF (ref)=%.6f" % marginRef.computePDF( NumericalPoint(2,0.25))
    print "margin CDF      =%.6f" % margin.computeCDF(NumericalPoint(2,0.25))
    print "margin CDF (ref)=%.6f" % marginRef.computeCDF(NumericalPoint(2,0.25))
    print "margin quantile      =", repr(margin.computeQuantile(0.95))
    print "margin quantile (ref)=", repr(marginRef.computeQuantile(0.95))
    print "margin realization=", repr(margin.getRealization())
    ...
    import sys
    print "t_SklarCopula.py", sys.exc_type, sys.exc_value
