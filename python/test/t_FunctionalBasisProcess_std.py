...
try :
    ...
    # Functional basis
    basisDimension = 10
    basis = NumericalMathFunctionCollection(basisDimension)
    coefficients =  DistributionCollection(basisDimension)

    for i in range(basisDimension):
        basis[i] = NumericalMathFunction("x", "sin(" + str(i) + "*x)")
        coefficients[i] = Normal(0.0, (1.0 + i))

    process = FunctionalBasisProcess(ComposedDistribution(coefficients), Basis(basis))
    ...
    import sys
    print "t_FunctionalBasisProcess_std.py", sys.exc_type, sys.exc_value
