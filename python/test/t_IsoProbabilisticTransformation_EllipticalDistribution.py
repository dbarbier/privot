...
try :
    ...
    print "inverse transform gradient at transformed point (FD)=" , repr(cleanMatrix(inverseTransform.gradient(transformedPoint)))
    print "inverse transform gradient at transformed point     =" , repr(cleanMatrix(CenteredFiniteDifferenceGradient(1.0e-5, inverseTransform.getEvaluationImplementation()).gradient(transformedPoint)))
