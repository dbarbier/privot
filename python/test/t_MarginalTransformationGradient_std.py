...
try :
    ...
    print "finite difference gradient(", repr(pointLow), ")=", repr(CenteredFiniteDifferenceGradient(1.0e-5, evaluation).gradient(pointLow))
    print "transformation.gradient(", repr(pointHigh), ")=", repr(transformation.gradient(pointHigh))
    print "finite difference gradient(", repr(pointHigh), ")=", repr(CenteredFiniteDifferenceGradient(1.0e-5, evaluation).gradient(pointHigh))
    ...
    print "finite difference gradient(", repr(uLow), ")=", repr(CenteredFiniteDifferenceGradient(1.0e-5, evaluation).gradient(uLow))
    print "transformation.gradient(", repr(uHigh), ")=", repr(transformation.gradient(uHigh))
    print "finite difference gradient(", repr(uHigh), ")=", repr(CenteredFiniteDifferenceGradient(1.0e-5, evaluation).gradient(uHigh))
    print "input dimension=", transformation.getInputDimension()
    print "output dimension=", transformation.getOutputDimension()
